...
import gst.python.gst_detection_small_obj as gst_detection
...
def test_tf_object_detection_model():
    ...
    with gst_detection.TfObjectDetectionModel(**config) as model:
        assert isinstance(model, gst_detection.TfObjectDetectionModel)

    with gst_detection.from_config_file("data/tf_object_api_cfg.yml"):
        assert isinstance(model, gst_detection.TfObjectDetectionModel)


def test_load_labels_from_file():
    labels = gst_detection.load_labels_from_file(
